...
- The shim's spray pool had taken VBO slot 247 from the menu and was
  overwriting its buffer every frame (the menu drew invisible). Main fixed it
  concurrently by moving spray to 246; the menu stays on 247.
- Merged over the fungus work: a loaded world keeps its saved fungus blocks
  (the wild patches are not re-stamped on load), but the mycelium field and
  the world's shown-species column field are not in the save and restart
  from the wild state. Persisting them is a follow-up.
