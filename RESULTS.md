...
1. **A library-defined type gets no deep drop (GAPS G71).** Type definitions
...
2. **A closure environment is freed shallowly (G72).** Every captured value
...
3. **A named binding unused in one arm of a lifted closure is never released
   (G73).** `Array.set`'s `lst_set` bound the replaced element as `h` and left
