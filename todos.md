...
- [ ] **Relight, the rest** — the sweep is no longer the biggest part of a relight: the clear, the seed, the before-copy and the section diff each pass over the 33-wide box (~0.3-0.5 ms each); a box sized to the edit's actual reach would cut all four.
- [ ] **Fungus, later** — effects are not saved; the map overlay is diagnostic-only, a player-facing territory map is still open; 132 live objects a frame remain on the frame path and are not the readout string.
