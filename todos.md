...
- [ ] **Weather: atmosphere, precipitation, weather actor** — design at `docs/superpowers/specs/2026-09-03-weather-design.md`: vertex layout 8 -> 9 floats (packed effect + alpha), exponential fog reusing the sky colour, `u_overcast`, a stateful particle pool killed by zero skylight, and a `WeatherActor` ticked on `tick_period()`. `CF_WEATHER`, `CF_PRECIP`.
